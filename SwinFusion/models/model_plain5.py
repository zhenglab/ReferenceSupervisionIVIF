...
class ModelPlain(ModelBase):
    ...
    def define_loss(self):
        ...
        elif G_lossfn_type == 'vif_5':            
            from models.loss_vif import fusion_loss_vif_5
            self.G_lossfn = fusion_loss_vif_5().to(self.device)

        elif G_lossfn_type == 'vif_5_average':            
            from models.loss_vif import fusion_loss_vif_5_avg
            self.G_lossfn = fusion_loss_vif_5_avg().to(self.device)
    ...
    def feed_data(self, data, need_GT=False, phase='Train'):
        ...
        self.GT_2 = data['GT_2'].to(self.device)
        self.GT_3 = data['GT_3'].to(self.device)
        self.GT_4 = data['GT_4'].to(self.device)
        self.GT_5 = data['GT_5'].to(self.device)
    # ----------------------------------------
    # feed L to netG
    # ----------------------------------------
    ...
    def optimize_parameters(self, current_step):
        ...
        elif G_lossfn_type in ['vif_5']:
            total_loss, loss_text, loss_int, loss_ssim = self.G_lossfn(self.A, self.B, self.E,self.GT, self.GT_2, self.GT_3,self.GT_4,self.GT_5)
            G_loss = self.G_lossfn_weight * total_loss      
        elif G_lossfn_type in ['vif_5_average']:
            total_loss, loss_text, loss_int, loss_ssim = self.G_lossfn(self.A, self.B, self.E,self.GT, self.GT_2, self.GT_3,self.GT_4,self.GT_5)
            G_loss = self.G_lossfn_weight * total_loss        
        else:
