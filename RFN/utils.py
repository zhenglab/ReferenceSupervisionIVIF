...
import numpy as np
import torch
from args_fusion_gt_9 import args
...
def list_images(directory):
    req_path_ir = directory+'.txt'
    images_ir = []
    names_ir = []
    lines = []
    
    with open(req_path_ir) as f:
        for line in f:
          lines.append(line)  
    ...
    for line in lines:
        line = line.strip('\n')
        line = line.split(',')
        line1 = directory+'/'+line[0]
        images_ir.append(line1)
        names_ir.append(line[1])
        names_ir = list(map(float, names_ir))
    return images_ir, names_ir
...
def get_test_image(paths, height=None, width=None, flag=False):
    ...
    for path in paths:
        if flag is True:
            ...
            image = imread(path, mode='L')
            # image, cb, cr = rgb_to_ycbcr(image)
        # get saliency part
        if height is not None and width is not None:
            image = imresize(image, [height, width], interp='nearest')
...
def recons_fusion_images_1(img_lists, h, w):
    img_f_list = []
    h_cen = int(np.floor(h / 2))
    w_cen = int(np.floor(w / 2))
    c = img_lists[0][0].shape[1]
...
def get_train_images(paths, weight,height=256, width=256, flag=False):
    if isinstance(paths, str):
        paths = [paths]
    images = []
    names = []
    for path in paths:
        ...
        images.append(image)
    for w in weight:
        name = np.reshape(w, [1, 1, 1])
        names.append(name)
    images = np.stack(images, axis=0)
    images = torch.from_numpy(images).float()
    names = np.stack(names, axis=0)
    names = torch.from_numpy(names).float()
    return images, names
