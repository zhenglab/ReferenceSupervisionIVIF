...
def make_data(sess, data, label,weight,data_dir,check_path):
  """
...
def input_setup(sess,config,data_path,data_dir,check_path,index=0):
  """
  Read image files and make their sub-images and saved them as a h5 file format.
  """
  # Load data path
  if config.is_train:
    data, weight = prepare_data(sess, dataset = data_path + data_dir)
  else:
    data, weight = prepare_data(sess, dataset = data_path + data_dir)
  ...
  if config.is_train:
    ...
    for i in range(len(data)):
      ...
      if len(input_.shape) == 3:
        ...
        h, w = input_.shape
        sub_input=cv2.resize(input_, (config.image_size,config.image_size),interpolation=cv2.INTER_CUBIC)
        sub_input = sub_input.reshape([config.image_size, config.image_size, 1])
        sub_label=cv2.resize(label_, (config.label_size,config.label_size),interpolation=cv2.INTER_CUBIC)
        sub_label = sub_label.reshape([config.label_size, config.label_size, 1])
        weight_tmp=weight1*len(sub_input)
        weight_patch.append(weight_tmp)
        sub_input_sequence.append(sub_input)
        sub_label_sequence.append(sub_label)
    sz = len(sub_input_sequence)
    labelsize = len(sub_label_sequence)
  ...
  #print(arrdata.shape)
  data_dir1=data_dir.split('.')
  make_data(sess, arrdata, arrlabel,arrweight,data_dir1[0],check_path)
