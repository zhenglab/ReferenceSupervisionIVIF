...
def generate(ir_path, vis_path, model_path, name_ir,index, output_path = None):
	ir_img=imageio.imread(ir_path,as_gray=True)/ 255.0
	vis_img = imageio.imread(vis_path,as_gray=True) / 255.0
	...
	with tf.Graph().as_default(), tf.Session() as sess:
		...
		output = output[0, :, :, 0]
		imageio.imsave(output_path + name_ir + '.jpg', output)
